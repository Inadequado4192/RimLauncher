...
export default function ModPackListWindowDialog() {
    ...
    const onValidateNames: PromptData["onValidate"] = (value) => {
        if (list.some(i => i.name.toLowerCase() == value.toLowerCase())) {
            return Localize("thisNameAlreadyUsed")
        } else return true;
    }
    ...
    return (
        <ModalDialog minWidth="md">
            ...
            <DialogContent>
                ...
                >
                    <thead>
                        <tr>
                            <th>{Localize("name")}</th>
                            <th>{Localize("countOfMods")}</th>
                            <th>{Localize("DLC")}</th>
                            <th>{Localize("gameVersion")}</th>
                        </tr>
                    </thead>
                    <tbody>
            ...
            <Divider />
            <DialogActions>
                <Button startDecorator={<FolderIcon />} onClick={async () => $invoke.openPath((await $invoke.getPathes()).Dir_ModPacks)}>{Localize("openDirectory")}</Button>
